...
namespace Winkel
{
    class DataStorageMetDataTable : DataStorage
    {
        ...
        private CustomerAdapter adapter; // nodig om dataTable aan te passen

        public DataStorageMetDataTable()
        {
            connection = GetConnection();
            adapter = new CustomerAdapter(base.dbProviderFactory, connection);
        ...
        public List<Customer> GetCustomersFromDataBase_WithUpdate()
        {
            // dit (table==null) is onwaarschijnlijk: in constructur vulde je table al op...
            if (table == null)
            {
                table = adapter.GetCustomersWithoutUpdate();
            }
            ...
            {
                table = adapter.UpdateAndGetCustomers(table);
            }

            return GetCustomersFromTable(table);
        }

        public void DuidCustomerAanOmWegTeHalen(String customerNumber)
        {
        ...
        public void ZetWijzigingenKlaarVoorCustomerWithNumber(String alleVelden, String alleWaarden, String customerNumber)
        {
        ...
        public void ZetWijzigingenKlaarVoorCustomer(String alleVelden, String alleWaarden, String keyColumn, String keyValue)
        {
            // Idem als hierboven.
            // Nu wordt er geen primary key meegegeven, maar moeten alle customers waarvoor keyColumn de gegeven
            // keyValue bevat, aangepast worden.
            // Vervang ook hier de vier vraagtekens.
            String[] velden = alleVelden.Split(';');
            String[] waarden = alleWaarden.Split(';');

            foreach (DataRow row in table.Rows)
            {
                if (row[keyColumn].Equals(keyValue))
                {
                    for (int i = 0; i < velden.Length; i++)
                    {
                        row[velden[i]] = waarden[i];
        ...
        public void ZetCustomerKlaarOmToeTeVoegen(Customer customer)
        {
